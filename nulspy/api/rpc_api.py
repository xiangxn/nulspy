...
class RpcAPI:
    ...
    async def _post(self, json, timeout=30):
        ...
        async with aiohttp.ClientSession(headers=self.headers) as session:
            try:
                async with session.post(self.url, json=json,
    ...
    async def getBalance(self, addr, asset_chain=2, asset=1):
        ...
        pars = self._create_pars("getAccountBalance",
                                 [self.chain_id, asset_chain, asset, addr])
        result = await self._post(pars)
        return result
    ...
    async def getAccountTxs(self,
                            ...
                            asset_chainId=0,
                            asset_id=0):
        pars = self._create_pars("getAccountTxs", [
            self.chain_id, page, page_size, addr, tx_type, start_block,
            end_block, asset_chainId, asset_id
        ])
        result = await self._post(pars)
        return result
